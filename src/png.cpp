...
readPNG(const char *filename)
{
	...
	if(state.info_raw.bitdepth == 4 && state.info_raw.colortype == LCT_PALETTE){
		...
		image->allocate();
		sceClibMemcpy(image->palette, state.info_raw.palette, state.info_raw.palettesize*4);
		...
		case LCT_PALETTE:
			...
			image->allocate();
			sceClibMemcpy(image->palette, state.info_raw.palette, state.info_raw.palettesize*4);
			sceClibMemcpy(image->pixels, raw, w*h);
			break;
		case LCT_RGB:
			image = Image::create(w, h, 24);
			image->allocate();
			sceClibMemcpy(image->pixels, raw, w*h*3);
			break;
		default:
		...
		case LCT_RGBA:
			...
			image->allocate();
			sceClibMemcpy(image->pixels, raw, w*h*4);
			break;
		}
	}
