...
copySkin(void *dst, void *src, int32 offset, int32)
{
	...
	*PLUGINOFFSET(Skin*, dst, offset) = dstskin;
	...
	sceClibMemcpy(dstskin->usedBones, srcskin->usedBones, srcskin->numUsedBones);
	sceClibMemcpy(dstskin->inverseMatrices, srcskin->inverseMatrices,
	       srcskin->numBones*64);
	sceClibMemcpy(dstskin->indices, srcskin->indices, geometry->numVertices*4);
	sceClibMemcpy(dstskin->weights, srcskin->weights, geometry->numVertices*16);
	return dst;
}
